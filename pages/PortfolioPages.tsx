...
import { achievements, community, currently, education, experience, projects, publications, researchInterests, site, writing } from '../content/site';
...
export function HomePage() {
  ...
  return <>
    ...
    <section className="home-intro section-pad-quiet reveal"><p className="lede">I enjoy the moment an idea becomes something real — across AI, agents, semantic systems, HCI, and immersive technology. <a className="text-link" href={pageHref('/about')}>Read my story <ArrowRight size={15} /></a></p></section>
    <section className="section-pad reveal"><SectionHeading index="01" title="What's new" /><div className="whatsnew-grid">
      ...
      <article className="whatsnew-card"><p className="eyebrow">Latest writing</p><h3>{latestPost.title}</h3><p>{latestPost.excerpt}</p><a className="text-link" href={pageHref(`/writing/${latestPost.slug}`)}>Read more <ArrowRight size={15} /></a></article>
    </div></section>
    <ContactCallout />
  </>;
}
...
export function AboutPage() {
  return <>
    ...
    <section className="section-pad reveal"><SectionHeading title="Community, leadership & volunteering" /><div className="timeline">{community.map((item) => <article key={`${item.period}-${item.title}`}><span>{item.period}</span><div><h3>{item.title}</h3><p className="eyebrow">{item.place}</p>{item.detail && <p>{item.detail}</p>}</div></article>)}</div><aside className="content-guide"><p className="eyebrow">Also planned for this section</p><p>Open-source contributions and talks/conference presentations will be added here as they become public-safe to share.</p></aside></section><ContactCallout />
  </>;
}

export function ProjectsPage() {
  const filters = ['All', 'AI systems', 'Research', 'Full-stack', 'Competition'];
...
function PublicationGroup({ index, title, items }: { index: string; title: string; items: typeof publications }) {
  if (!items.length) return null;
  return <section className="section-pad reveal"><SectionHeading index={index} title={title} /><div className="publication-list">{items.map((publication) => <article key={publication.slug}><span>{publication.date || publication.year}</span><div><p className="eyebrow">{publication.typeNote || publication.venue}</p><h2>{publication.title}</h2><p className="publication-venue">{publication.venue}</p>{publication.authors && <p>{publication.authors}</p>}<p>{publication.summary}</p><div className="inline-links">{publication.paper && <a href={publication.paper} target="_blank" rel="noreferrer">Paper {linkIcon}</a>}{publication.proceeding && <a href={publication.proceeding} target="_blank" rel="noreferrer">Proceedings {linkIcon}</a>}{publication.presentation && <a href={publication.presentation}>Presentation {linkIcon}</a>}</div><PlaceholderNote items={publication.needs} compact /></div></article>)}</div></section>;
}
...
export function ResearchPage() {
  ...
  return <><PageIntro eyebrow="Research · Questions in progress" title="Intelligent systems should be useful, understandable, and built around people."><p>My interests sit between AI, semantic knowledge, interaction, accessibility, and immersive systems—with research as a way to ask better questions and prototypes as a way to test them.</p></PageIntro>
    <section className="section-pad reveal"><SectionHeading index="01" title="Research interests" /><ul className="interest-list">{researchInterests.map((interest) => <li key={interest}>{interest}</li>)}</ul></section>
    <PublicationGroup index="02" title="Published & presented" items={published} />
    <PublicationGroup index="03" title="Accepted — presentation & publication in progress" items={accepted} />
    <PublicationGroup index="04" title="Under review & ongoing research" items={underReview} />
    <section className="section-pad reveal"><SectionHeading index="05" title="Conference presentations & talks" /><div className="empty-state"><p className="eyebrow">Presentations & conferences</p><h2>Ready for talks, posters, and conference activity.</h2><p>Add event, date, presentation title, format, and material links.</p></div></section><ContactCallout /></>;
}
